...
class Sync(Command):
    ...
    async def run(self, options, *args, **kwargs):
        ...
        solutions = load_solutions(
            root_dir,
            solution_file,
            ignore_non_existing=options.compatible,
        ...
        for solution in solutions:
            if options.main:
                ...
                await solution.fetch_deps_only(
                    root_dir, options, {}, existing_targets={root_dir: solution}
                )
            ...
            for dep in solution.list_deps():
                ...
                if not options.disable_ignore and dep.ignore_in_git and dep.parent:
                    ignore_paths_in_git(
                        dep.parent.target_dir, [dep.target_dir], ignore_errors=True
                    )

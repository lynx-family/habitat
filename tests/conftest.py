...
def fixture_default_repo(tmp_path):
    ...
    """
    subprocess.run(
        shlex.split("git init main --initial-branch=master"), cwd=tmp_path, check=True
    )
    subprocess.run(
        shlex.split("git init git-dependency --initial-branch=master"),
        cwd=tmp_path,
        check=True,
    )
...
def fixture_lfs_repo(default_repo):
    ...
    subprocess.run(
        shlex.split("git init lfs --initial-branch=master"), cwd=remote_path, check=True
    )

    # we use temp_path/remote/lfs as a lfs server, use temp_path/lfs to push binary file to lfs server.
    subprocess.run(
        shlex.split(f"git clone file://{remote_path}/lfs/.git"),
        cwd=temp_path,
        check=True,
    )
    ...
    subprocess.run(
        shlex.split("git checkout -b main"), cwd=temp_lfs_repo_path, check=True
    )
    subprocess.run(
        shlex.split("git lfs track '/binary'"), cwd=temp_lfs_repo_path, check=True
    )
    subprocess.run(shlex.split("git add ."), cwd=temp_lfs_repo_path, check=True)
    subprocess.run(
        shlex.split('git commit -m "track binary with git lfs"'),
        cwd=temp_lfs_repo_path,
        check=True,
    )
    subprocess.run(
        shlex.split("git push origin main"), cwd=temp_lfs_repo_path, check=True
    )

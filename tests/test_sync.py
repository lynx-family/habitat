...
def test_sync_recursively_duplicated_source():
    ...
    os.chdir("../..")  # repos/git3 -> .
    run_with_custom_argv(
        main, ["hab", "config", f"file://{cwd}/repos/git1/.git", "main", "-b", "master"]
    )
    ...
    assert os.path.realpath(
        os.path.join("main", "sub1", "subsub2")
    ) == os.path.realpath(os.path.join("main", "sub2"))
...
def disabled_test_sync_http_dependency(httpserver, default_repo):
    ...
    httpserver.expect_request(
        uri="/download/binary.zip", method="HEAD"
    ).respond_with_data(
        status=200,
        headers={
    ...
    httpserver.expect_request(
        uri="/download/binary.zip", method="GET"
    ).respond_with_data(status=200, response_data=data)
...
def test_sync_target_only(tmp_path):
    ...
    make_change_in_repo(
        f"{cwd}/test-repo",
        "DEPS.android",
        generate_habitat_config_file("deps", android_deps),
        "add android deps",
        "w",
    )
    make_change_in_repo(
...
def test_sync_git_repo_with_patches(tmp_path):
    ...
    first_commit = make_change_in_repo(
        f"{cwd}/lib", "hello.py", 'print("hello, world.")', "add hello world", "w"
    )

    make_change_in_repo(
        f"{cwd}/lib", "hello.py", '\nprint("thanks!")', "add thanks", "a"
    )

    make_change_in_repo(f"{cwd}/lib", "hello.py", 'print("done")', "done", "w")

    make_change_in_repo(f"{cwd}/lib", "hello.py", 'print("not yet")', "not yet", "w")
    ...
    subprocess.check_call(
        [
            "mv",
            f"{cwd}/lib/0001-add-thanks.patch",
            f"{cwd}/main/patches/0001-add-thanks.patch",
        ]
    )
    subprocess.check_call(
        ["mv", f"{cwd}/lib/0002-done.patch", f"{cwd}/main/patches/0002-done.patch"]
    )
    subprocess.check_call(
        [
            "mv",
            f"{cwd}/lib/0003-not-yet.patch",
            f"{cwd}/main/other_patches/0003-not-yet.patch",
        ]
    )
    os.chdir("main")
    subprocess.check_call(["git", "add", "."])
